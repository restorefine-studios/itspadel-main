    ...
    <section className="bg-[#111111] py-20 px-8 md:px-16 lg:px-32 relative text-white pt-32">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-16 gap-6">
        ...
        <div className="text-right">
          ...
          <p className="font-inter text-gray-400 font-light mt-2 max-w-md ml-auto">
            {data.introDesc}
          </p>
        </div>
      </div>
        ...
        {data.categories.map((cat) => (
          <button
            ...
            className={`transition-colors px-4 py-2 ${
              activeTab === cat.id ? 'text-white border-b-4 border-[#009FF3] pb-1' : 'text-gray-600 hover:text-gray-300'
            }`}
          >
      ...
      <div className="mb-12 animate-fade-in-0">
        <p className="font-inter text-gray-300 font-light max-w-3xl leading-relaxed whitespace-pre-line">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-1">
        {activeCategory.days.map((day, idx) => (
          <div 
            key={idx} 
            className="border-[2px] bg-[#1a1a1a]/50 p-6 flex flex-col relative overflow-hidden transition-all hover:bg-[#1a1a1a]"
          ...
          >
            ...
            <div className="relative z-10 mb-8 pl-2">
              <h3 className="inline-block font-bison font-bold text-5xl text-white tracking-widest" style={{ transform: 'skewX(-18deg)' }}>{day.name}</h3>
            ...
            <div className="flex flex-col gap-6 relative z-10 flex-grow">
              {day.sessions.map((session, sIdx) => (
                <div key={sIdx} className="flex flex-col border-b border-gray-800/50 pb-4 last:border-0 last:pb-0">
                  ...
                  <div className="mt-2 text-sm font-bison italic tracking-widest text-gray-500">
                    START: {session.time}
                  </div>
                </div>
              ))}
            ...
            {day.extra && (
              <div className="mt-6 pt-4 border-t border-gray-800 text-xs italic text-gray-500 font-inter">
                *{day.extra}
              </div>
            )}
          </div>
