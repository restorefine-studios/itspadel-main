...
const CoachProfile = ({ data }: CoachProfileProps) => {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-black pb-16 pt-8">
      ...
      <div 
        className="grid grid-cols-1 lg:grid-cols-2 min-h-[600px] lg:min-h-screen bg-white relative"
        style={{ clipPath: "polygon(0 0, 100% 3vw, 100% calc(100% - 4vw), 0 100%)" }}
      >
        ...
        <div className="relative bg-[#050505] overflow-hidden flex items-end justify-start min-h-[500px] lg:min-h-screen">
          ...
          <div 
            key={`${currentCoach.name}-banner`}
            className="relative z-10 bg-[#009FF3] pl-6 md:pl-10 pr-16 md:pr-24 py-8 mb-[3vw] animate-fade-in-1"
            style={{ clipPath: "polygon(0 0, 100% 0, 85% 100%, 0% 100%)" }}
          >
            <h3 
              className="inline-block font-bison font-bold text-white text-5xl md:text-6xl lg:text-[70px] tracking-tight uppercase leading-none mb-1"
              style={{ transform: 'skewX(-18deg)' }}
            >
              {currentCoach.name}
            </h3>
            <p className="font-inter font-bold text-white text-[10px] md:text-xs tracking-[0.2em] uppercase">
              {currentCoach.title.replace(/\+/g, '•')}
            </p>
          </div>
        </div>
        ...
        <div className="relative bg-white flex flex-col justify-center py-20 px-8 md:px-16 lg:px-24">
          ...
          <div className="absolute top-10 right-0 pointer-events-none select-none opacity-[0.03] overflow-hidden w-full flex justify-end">
            <span className="font-bison font-bold text-[100px] md:text-[200px] lg:text-[250px] leading-none text-black mr-[-5%] tracking-tight">
              MASTER
            </span>
          </div>
          ...
          <div key={`${currentCoach.name}-content`} className="relative z-10 max-w-xl animate-fade-in-2">
            ...
            <div className="mb-6">
              <span 
                className="inline-block font-bison font-bold text-[#009FF3] text-6xl md:text-8xl"
                style={{ transform: 'skewX(-18deg)' }}
              >
            ...
            {/* Quote Heading */}
            <h2 className="font-bison font-black uppercase text-5xl md:text-6xl lg:text-[70px] leading-[0.9] text-black mb-8 tracking-tight">
              {currentCoach.quote.heading}
            </h2>

            {/* Quote Text */}
            <div className="mb-10 min-h-[80px]">
              <p className="font-inter text-lg md:text-xl text-gray-700 leading-relaxed font-medium">
                &quot;{currentCoach.quote.text}&quot;
              </p>
            </div>
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 mb-16">
              ...
              <div className="flex gap-16 md:gap-24">
                <div>
                  <div 
                    className="font-bison font-bold text-4xl md:text-5xl text-black inline-block"
                    style={{ transform: 'skewX(-15deg)' }}
                  >
                    {currentCoach.stats.proTitles}
                  </div>
                  <div className="font-inter font-bold text-[10px] md:text-xs tracking-[0.2em] uppercase text-gray-500 mt-2">
                    PRO TITLES
                  </div>
                </div>
                <div>
              ...
              <div className="flex gap-4">
                <button 
                  onClick={handlePrev}
                  className="w-14 h-14 flex items-center justify-center bg-black hover:bg-[#009FF3] text-white transition-all group shadow-md hover:shadow-xl"
            ...
            <div className="space-y-4 text-gray-600 mt-12 hidden lg:block">
               <p className="font-inter text-sm italic border-l-2 border-[#009FF3] pl-4">
                 {currentCoach.description[0]}
               </p>
            </div>
